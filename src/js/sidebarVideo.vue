<template>
<div id="sidebar">
  <el-button @click="getVideo" type="primary" style="position:relative;z-index:10;top:51px">选择视频</el-button>
  <el-drawer id="drawerBox"
    title="上传和选择视频"
    :visible.sync="drawer"
    :direction="'ltr'"
    :with-header="false">
    <uploadVideo @changeUpload="changeUpload"></uploadVideo>
    <el-row :gutter="20">
      <el-col :span="6" v-for="(item,index) in videoSrcList" :key="item.id" @mouseenter.native="mouseEnter(index)"  @mouseleave.native="mouseLeave(index)" >
        <div class="demo-image__preview" :ref="itemName">
          <span id="wh">{{item.width*4}}*{{item.height*4}}</span>
...
export default {
  ...
  methods: {
          ...
          .then((res) => {
            ...
            res.fileList.forEach((el) => {
              if (el.code === "SUCCESS") {
                ...
                this.getTotal();
                if (_this.pageNum==1) {
                  _this.videoSrcList.pop();
                  _this.videoSrcList.unshift({url:el.tempFileURL,time:src[0].time,width:src[0].width,height:src[0].height,poster:src[0].poster});
                }
    ...
    getTotal(){
      var _this = this;
      db.collection("videoData").where({_openid: _.eq(_this.userId)}).get().then((res)=>{
    ...
    getVideo(){
      ...
      async function getData() {
        var imgData = await db.collection("videoData").where({_openid: _.eq(_this.userId)})
        ...
        for (let index = 0; index < imgData.length; index++) {
          ...
          .then((res) => {
            ...
            res.fileList.forEach((el) => {
              if (el.code === "SUCCESS") {
                _this.videoSrcList.push({
                  url:el.tempFileURL,
                  time:imgData[index].time,
                  width:imgData[index].width,
                  height:imgData[index].height,
                  poster:imgData[index].poster
                  });
                // console.log(el.tempFileURL)
              } else {
                //获取下载链接失败
